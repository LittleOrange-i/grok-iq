...
支持按条件定位账号，在当前筛选结果中多选、全选并一键创建测试。任务执行时，不同账号可以并行处理；同一账号则保持有序，避免相互干扰。

### 两类测试，覆盖不同场景

完整对话测试用于观察回答质量、指令遵循、输出内容和预期结果；快速质量测试适合在多个出口间快速筛查表现。每次执行都保留关键指标和响应证据，方便复盘。

### 可追溯的风险结论

单次异常只是一条信号。系统会结合响应速度、生成表现、自动校验、连续异常次数和跨出口情况给出判断，避免把偶发波动直接当成最终结论。

### 任务、计划与运行状态一处查看

手动创建的任务和定时计划统一进入队列。任务中心可查看状态与进度；Worker 页面可查看实际并发、当前工作和近期日志，排查问题更直接。

### 便于核对的样本详情

账号详情与任务详情会汇总最近样本、分类结果、指标和响应内容。长内容默认收起，需要时再打开或进入预览，既保持页面清爽，也不丢失细节。
